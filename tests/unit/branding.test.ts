...
const HOSTS_DECLARADOS: Record<string, EntradaDeHost> = {
  ...
  // `VERDASH_FUNCTIONS_URL`; os hosts abaixo são o fallback de cada uma. A instalação que
  // quiser outro endereço põe a variável — o que está no código é para onde o canal
  // aponta quando ninguém disse nada, e sem ele uma variável vazia produziria URL sem
  // host, que falha mais tarde e pior.
  //
  // Categoria FORNECEDOR pela mesma razão do `api.apify.com` logo abaixo: é o destino do
  // request, não palavra de interface. Não é o caso do #266 — ali o problema era domínio
  // pessoal de alguém preso no código que o cliente instala; aqui é o endereço do serviço
  // com que este canal conversa, e trocá-lo pelo domínio de um revendedor faria a chamada
  // não chegar a lugar nenhum.
  "fzap2.verdash.com.br": {
    categoria: "FORNECEDOR",
    motivo:
      "default de `VERDASH_API_BASE_URL`: a API da plataforma que roteia o canal hospedado. O `2` no nome é medido, não estilo — o host sem número responde 503, e fallback apontando para servidor morto falha parecendo configurado.",
  },
  "supabase.verdash.com.br": {
    categoria: "FORNECEDOR",
    motivo:
      "default de `VERDASH_FUNCTIONS_URL`: as edge functions da mesma plataforma, que recebem o pareamento e devolvem a credencial da linha. Mesmo motivo: é o destino do request.",
  },
