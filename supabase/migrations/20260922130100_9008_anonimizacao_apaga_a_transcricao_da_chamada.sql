-- 9008 — a anonimizacao de LGPD volta a apagar o que ela promete apagar.
--
-- ─── Os DOIS defeitos, e por que eles sao da mesma familia ──────────────────
-- `fn_lgpd_cascade_redact_contact` e reemitida DEZ vezes ao longo do baseline e
-- da cadeia, e `create or replace` faz a ultima vencer. Duas reemissoes
-- recentes do upstream copiaram um corpo ANTIGO por cima de consertos que ja
-- estavam feitos — e o que se perdeu, em cada caso, foi um passo inteiro:
--
--  1. `voice_calls.transcript` (acrescentado pela 0348). O baseline documenta o
--     conserto em letras — "aqui so o UPDATE de voice_calls ganha
--     `transcript = null` a mais" — e a definicao vigente nao o tinha mais.
--     `transcript` guarda o que foi DITO na chamada: quase sempre o nome da
--     pessoa, falado em voz alta, as vezes o endereco.
--
--  2. O passo de `sales` (acrescentado pela 0359): `notes`, `cancel_reason` e
--     `reverse_reason` da comanda, que sao campos de texto livre onde o
--     atendente escreve o nome e o motivo.
--
-- O efeito dos dois e o mesmo, e e a pior forma de falha de LGPD: o titular
-- exerce o direito ao apagamento, a rota devolve SUCESSO, os contadores fecham,
-- o SLA e marcado como cumprido — e o dado continua legivel, amarrado ao
-- `contact_id`. Falha silenciosa com recibo de conformidade por cima. Ninguem
-- confere de novo uma coisa que disse ter dado certo.
--
-- ─── Por que a definicao vem INTEIRA, e nao por ancora ──────────────────────
-- A primeira versao deste conserto lia a definicao vigente do catalogo e
-- trocava um trecho por outro. Funcionava no banco e deixava um buraco no
-- processo: `tests/unit/apendice-do-baseline-nao-diverge-da-cadeia` compara a
-- ULTIMA definicao da CADEIA com a do apendice do BASELINE, e um conserto
-- dinamico nao e uma definicao — o gate continuaria vendo a cadeia divergir do
-- baseline sem nada a apontar. Foi o proprio gate que mostrou isso.
--
-- Entao aqui vai o corpo completo, identico ao do apendice do baseline. Se o
-- upstream reemitir a funcao de novo e perder um passo outra vez, o gate acusa
-- a divergencia no mesmo dia em vez de meses depois, numa auditoria.
--
-- Editar a 0348, a 0359 ou a 0370 no lugar disto e proibido: elas ja rodaram na
-- base de quem acompanha o upstream, e migration que ja rodou nao se reescreve.
--
-- ─── A causa raiz tambem foi consertada ─────────────────────────────────────
-- O gate procurava `create or replace function public.fn_...` com `public` SEM
-- aspas. Todas as reemissoes desta funcao vem na forma do `pg_dump`
-- (`"public"."fn_..."`), entao a funcao mais sensivel do produto NUNCA entrou
-- na varredura. A regex agora aceita as duas grafias
-- (`tests/unit/apendice-do-baseline-nao-diverge-da-cadeia.test.ts`), e e ela
-- que faz o proximo caso ser pego sozinho.
